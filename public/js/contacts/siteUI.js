let ContactAPI = undefined;

import("../API.js").then(m => {
    ContactAPI = new m.API("http://localhost:5000/api/contacts");
    Init_UI();
});
...
async function renderEditContactForm(id) {
    showWaitingGif();
    let contact = await ContactAPI.get(id);
    if (contact !== null)
...
async function renderDeleteContactForm(id) {
    ...
    if (contact !== null) {
        ...
        $('#deleteContact').on("click", async function () {
            showWaitingGif();
            let result = await ContactAPI.delete(contact.Id);
            if (result)
                renderContacts();
            else
...
function renderContactForm(contact = null) {
    ...
    $('#contactForm').on("submit", async function (event) {
        ...
        showWaitingGif();
        let result = await ContactAPI.save(contact, create);
        if (result)
            renderContacts();
        else
            renderError("Une erreur est survenue!");
